...
import { computeStats, incrementMessageCount, getPlanModel, canSendMessage } from '@/lib/messageLimit';
...
export default function Kael() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    base44.entities.KaelMessage.filter({}, 'created_date', 200).then(msgs => {
      setMessages(msgs);
    });
  }, [user?.id]);
  ...
  const sendMessage = async () => {
    ...
    if (isEditing) {
      ...
      await Promise.all(toDelete.map(m => base44.entities.KaelMessage.delete(m.id)));
    ...
    const userMsg = await base44.entities.KaelMessage.create({
      role: 'user',
      content: text,
      is_edit: isEditing,
    });
    ...
    const model = getPlanModel(plan);

    // Call AI with plan-based model
    const response = await base44.integrations.Core.InvokeLLM(
      model ? { prompt, model } : { prompt }
    );

    // Create assistant message in DB
    const assistantMsg = await base44.entities.KaelMessage.create({
      role: 'assistant',
      content: response,
    });
