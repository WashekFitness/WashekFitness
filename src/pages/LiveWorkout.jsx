...
function MovementBaseline({ trainingType }) {
  ...
  const { data: baselines = [] } = useQuery({
    queryKey: ['baselines'],
    queryFn: () => base44.entities.MovementBaseline.list('-recorded_date', 1),
  });
  ...
  const saveBaseline = async () => {
    ...
    if (baselines[0]?.recorded_date === today) {
      await base44.entities.MovementBaseline.update(baselines[0].id, data);
    } else {
      await base44.entities.MovementBaseline.create(data);
    }
...
Respond in JSON:
{
  ...
  "adjusted_exercises": [{ "name": "...", "sets": 0, "reps": "...", "rest_seconds": 0, "notes": "..." }]
      ...
      if (nextMicrocycle && result.adjusted_exercises?.length > 0 && onProgramUpdated) {
        ...
        await base44.entities.WorkoutProgram.update(program.id, { microcycles: updatedMicrocycles });
...
export default function LiveWorkout() {
  ...
  const { data: programs = [] } = useQuery({
    queryKey: ['programs'],
    queryFn: () => base44.entities.WorkoutProgram.filter({ status: 'active' }, '-created_date', 1),
  });
