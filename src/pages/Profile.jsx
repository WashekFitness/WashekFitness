...
export default function Profile() {
  ...
  const { data: logs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.WorkoutLog.filter({ created_by: user.email }, '-date', 999),
    enabled: !!user?.email,
  });
  ...
  const handleRegenerate = async () => {
    ...
    try {
      if (activeProgram) {
        await base44.entities.WorkoutProgram.update(activeProgram.id, { status: 'completed' });
      }
      navigate('/onboarding');
    } finally {
  ...
  return (
    <div className="px-5 safe-bottom space-y-4 pt-2">
      ...
      <div className="space-y-2">
        ...
        <Button variant="ghost" className="w-full h-12 justify-start text-destructive hover:text-destructive" onClick={() => base44.auth.logout()}>
