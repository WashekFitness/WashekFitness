      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 w-full bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-2 border-b border-border">
            <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="w-full h-9 px-3 text-sm bg-muted rounded-xl outline-none placeholder:text-muted-foreground" />
          </div>
          <div className="max-h-52 overflow-y-auto">
            {filtered.map(opt => (
              <button key={opt} onClick={() => { onChange(opt); setOpen(false); setSearch(''); }}
                className={cn('w-full px-4 py-2.5 text-sm text-left hover:bg-muted/80 transition-all', opt === value && 'bg-primary/10 text-primary font-semibold')}>
                {opt}
              </button>
            ))}
            {filtered.length === 0 && <p className="px-4 py-3 text-xs text-muted-foreground">No results</p>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full h-12 px-4 rounded-2xl border-2 border-border bg-card text-sm text-left flex items-center justify-between hover:border-muted-foreground/30 transition-all"
      >
        <span className={selected ? 'text-foreground' : 'text-muted-foreground'}>{selected?.name || 'Select your country…'}</span>
        <span className="text-muted-foreground text-xs">▾</span>
      </button>
      {open && (
        <div className="absolute z-50 top-full mt-1 w-full bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-2 border-b border-border">
            <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search country..." className="w-full h-9 px-3 text-sm bg-muted rounded-xl outline-none placeholder:text-muted-foreground" />
          </div>
          <div className="max-h-52 overflow-y-auto">
            {filtered.map(c => (
              <button key={c.code} onClick={() => { onChange(c.code); setOpen(false); setSearch(''); }}
                className={cn('w-full px-4 py-2.5 text-sm text-left hover:bg-muted/80 transition-all', c.code === value && 'bg-primary/10 text-primary font-semibold')}>
                {c.name}
              </button>
            ))}
            {filtered.length === 0 && <p className="px-4 py-3 text-xs text-muted-foreground">No results</p>}
...
export default function Onboarding() {
  ...
  const toggleGoal = (val) => {
    setFitnessGoals(prev => prev.includes(val) ? prev.filter(g => g !== val) : [...prev, val]);
  };

  const toggleWeightGoal = (val) => {
    setWeightGoals(prev => prev.includes(val) ? prev.filter(g => g !== val) : [...prev, val]);
  };

  const hasSkills = trainingType === 'calisthenics' || trainingType === 'weighted_calisthenics' || trainingType === 'hybrid';
  const hasWeightGoals = trainingType === 'weights' || trainingType === 'hybrid';
  ...
  const runProgressTo = (target) => {
    ...
    progressTimer.current = setInterval(() => {
      setProgress(p => {
        ...
        return Math.min(p + Math.max(remaining * 0.04, 0.1), target - 0.5);
      });
    }, 400);
  };
...
const handleGenerate = async () => {
  ...
  const step3Valid = hasSkills
    ? (level && (!hasWeightGoals || weightGoals.length > 0))
    : weightGoals.length > 0;

  // Step 4 (generate) condition
  const step4Valid = hasSkills
    ? (goalDescription.trim().length >= 10 && equipment.trim().length > 0)
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            <h1 className="font-heading text-4xl font-bold mb-2 tracking-tight">
              Welcome to <span className="text-primary">Washek Fitness</span>
            </h1>
            ...
            <div className="flex-1 flex flex-col justify-center gap-4">
              ...
              <div>
                <p className="text-xs text-muted-foreground mb-1 font-medium">First Name</p>
                <Input
              ...
              <div>
                <p className="text-xs text-muted-foreground mb-1 font-medium">Last Name</p>
                <Input
              ...
              <div>
                ...
                <SearchSelect value={language} onChange={setLanguage} options={LANGUAGES} placeholder="Select language…" />
              </div>
              <div>
                <p className="text-xs text-muted-foreground mb-1 font-medium">Measurement System</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button key={v} onClick={() => setUnit(v)}
                      className={cn('h-11 rounded-2xl border-2 text-sm font-semibold transition-all',
                        unit === v ? 'border-primary bg-primary/10 text-foreground' : 'border-border bg-card text-muted-foreground hover:border-muted-foreground/30')}>
                      {label}
                    </button>
                  ))}
                </div>
        ...
        {step === 1 && (
          <motion.div
            ...
            className="flex-1 flex flex-col px-6"
          >
            <h2 className="font-heading text-2xl font-bold mb-1">Choose Your Path</h2>
            <p className="text-muted-foreground mb-6">What type of training are you here for, {firstName || 'Athlete'}?</p>
            <div className="flex-1">
              <TrainingTypeSelect value={trainingType} onChange={setTrainingType} />
            </div>
            <div className="flex gap-3 mb-8 mt-4">
              <Button variant="outline" size="lg" className="h-14" onClick={() => setStep(0)}>
        ...
        {step === 2 && (
          ...
          >
            ...
            <p className="text-muted-foreground mb-6">Your stats help us personalize nutrition goals and training load.</p>

            <div className="space-y-4 flex-1">
              <div>
                <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wider">Gender</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" placeholder="e.g. 24" value={age} onChange={e => setAge(e.target.value)} className="h-12 text-base" />
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1 font-medium">Weight ({unit === 'metric' ? 'kg' : 'lbs'})</p>
                  <Input type="number" placeholder={unit === 'metric' ? 'e.g. 80' : 'e.g. 175'} value={weightLbs} onChange={e => setWeightLbs(e.target.value)} className="h-12 text-base" />
                </div>
              </div>

              <div>
                <p className="text-xs text-muted-foreground mb-1 font-medium">Height</p>
                {unit === 'metric' ? (
                  <Input type="number" placeholder="Height in cm (e.g. 178)" value={heightFt} onChange={e => setHeightFt(e.target.value)} className="h-12 text-base" />
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    <Input type="number" placeholder="Feet (e.g. 5)" value={heightFt} onChange={e => setHeightFt(e.target.value)} className="h-12 text-base" />
                    <Input type="number" placeholder="Inches (e.g. 10)" value={heightIn} onChange={e => setHeightIn(e.target.value)} className="h-12 text-base" />
                  </div>
                )}
              </div>
              ...
              {hasSkills && (
                <div>
                  <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wider">Your Goals (select all that apply)</p>
                  <div className="grid grid-cols-2 gap-2">
                    {CALISTHENICS_GOALS.map(({ value, label, iconName }) => {
                      const GoalIcon = GOAL_ICONS[iconName] || Dumbbell;
                      return (
                        <button
                          key={value}
                          onClick={() => toggleGoal(value)}
                          aria-pressed={fitnessGoals.includes(value)}
                          className={cn(
            ...
            <div className="flex gap-3 mb-8 mt-4">
              ...
              <Button
                size="lg"
                className="flex-1 h-14 text-lg font-heading font-semibold"
                disabled={!gender || (hasSkills && fitnessGoals.length === 0)}
                onClick={() => setStep(3)}
              >
                Continue <ChevronRight className="ml-2 w-5 h-5" />
              </Button>
        ...
        {step === 3 && (
          ...
          >
            {hasSkills && (
              <>
                <h2 className="font-heading text-2xl font-bold mb-1">Your Level</h2>
                <p className="text-muted-foreground mb-6">Where are you in your journey, {firstName || 'Athlete'}?</p>
                <div className="space-y-3 flex-1">
                  {levels.map(({ value, label, desc, icon: Icon, placeholder }) => (
                    <div key={value}>
                      <button
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            level === value ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                          )}>
                      ...
                      {level === value && (
                        <div className="mt-1.5 px-1">
                          ...
                          />
                          <p className="text-xs text-muted-foreground mt-1 pl-1">What skills & moves can you currently do?</p>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                {hasWeightGoals && (
                  <div className="mt-6">
                    <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wider">Weight Training Goals (select all that apply)</p>
                    <div className="grid grid-cols-2 gap-2">
                      {WEIGHT_GOALS.map(({ value, label, iconName }) => {
                        const GoalIcon = GOAL_ICONS[iconName] || Dumbbell;
                        return (
                          <button
                            key={value}
                            onClick={() => toggleWeightGoal(value)}
                            aria-pressed={weightGoals.includes(value)}
                            className={cn(
                              'flex items-center gap-2 p-3 rounded-2xl border-2 text-left transition-all',
            ...
            {!hasSkills && (
              <>
                <h2 className="font-heading text-2xl font-bold mb-1">Your Goals</h2>
                <p className="text-muted-foreground mb-6">What do you want to achieve with weight training, {firstName || 'Athlete'}?</p>
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="font-heading text-2xl font-bold">
                {hasSkills ? `Tell us your goals, ${firstName || 'Athlete'}` : `Final details, ${firstName || 'Athlete'}`}
              </h2>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3">
              {hasSkills && (
                <>
                  ...
                  <div className="bg-muted/50 rounded-2xl p-4 border border-border">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">⏱ Timeframe for your goals</p>
                    <Textarea
                      value={timeframe}
              ...
              {!hasSkills && (
                <div className="bg-muted/50 rounded-2xl p-4 border border-border">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">📋 Goals summary</p>
                  <p className="text-sm text-muted-foreground">
                    You selected: <span className="font-semibold text-foreground">{weightGoals.join(', ') || 'General fitness'}</span>
                  </p>
                </div>
              )}

              <div className="bg-muted/50 rounded-2xl p-4 border border-border">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">🏋️ Available equipment <span className="text-destructive">*</span></p>
                <Textarea
                  value={equipment}
                  onChange={e => setEquipment(e.target.value)}
                  placeholder={hasSkills
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1.5">List everything you have access to — this is required.</p>
              </div>

              <div className="bg-muted/50 rounded-2xl p-4 border border-border">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">📝 Requirements & Notes</p>
                <Textarea
                  value={requirements}
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1.5">Time available, injuries, limitations, areas to focus on — anything that helps us make your program perfect.</p>
              </div>
            </div>

            <div className="flex gap-3 mb-8 mt-4">
              <Button variant="outline" size="lg" className="h-14" onClick={() => setStep(3)}>
                <ChevronLeft className="w-5 h-5" />
              </Button>
              <Button
                size="lg"
              ...
              >
                {loading ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="truncate">{loadingPhase || 'Building your program…'} {Math.round(progress)}%</span>
                  </div>
                ) : (
                  <>Build My Program <Sparkles className="ml-2 w-5 h-5" /></>
                )}
              </Button>
            </div>
            {loading && (
              <div className="mb-6 space-y-2">
                ...
                <p className="text-xs text-muted-foreground text-center">{loadingPhase || 'Building your program…'}</p>
              </div>
            )}
          </motion.div>
        )}
