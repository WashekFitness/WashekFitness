...
export default function ProgressPhotos() {
  ...
  const { data: photos = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ProgressPhoto.filter({ created_by: user.email }, '-date', 100),
    enabled: !!user?.email,
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.ProgressPhoto.create(data),
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.ProgressPhoto.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['progress-photos'] }),
  });
