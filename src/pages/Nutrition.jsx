...
export default function Nutrition() {
  ...
  const { data: entries = [] } = useQuery({
    ...
    queryFn: () => base44.entities.NutritionEntry.filter({ date: today, created_by: user.email }, '-created_date', 100),
    enabled: !!user?.email,
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.NutritionEntry.create({ ...data, date: today, meal_type: data.meal_type || mealType }),
    onMutate: async (data) => {
